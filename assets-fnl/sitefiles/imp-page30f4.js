...
$(document).on('click', 'a[href^="#"]', function(event) {
    try {
        ...
        if ($(window).width() >= 992) {
            $('html, body').animate({
                scrollTop: $($('body').find(href)).offset().top - minus
            }, 400);
        } else {
            $('html, body').animate({
                scrollTop: $($('body').find(href)).offset().top - minus
...
$(document).on('click', '.get-custom-leads', function(e) {
    ...
    if ($.trim($formj.find('input[name="full-name"]').val()) == '') {
        if ($formj.find('input[name="full-name"]').attr('required') == 'required') {
            ...
            $formj.find('input[name="full-name"]').focus();
            return false;
        }
    }
    ...
    if ($.trim($formj.find('input[name="first-name"]').val()) == '') {
        if ($formj.find('input[name="first-name"]').attr('required') == 'required') {
            send_message('error', 'Oops! Name fileld is required');
            $formj.find('input[name="first-name"]').focus();
            return false;
        }
    }
...
if ($('.obj-fb-comments').length > 0) {
    ...
    $('.obj-fb-comments').find('iframe, span, .fb_iframe_widget').css({
        'width': '100%',
        'height': '100vh',
        'left': 0,
        'overflow': 'visible'
    });
    $('.obj-fb-comments').find('._4k-6').css({
        'overflow': 'auto',
        'height': '100vh'
    });
}
