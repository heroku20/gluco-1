$(document).ready(function(e) {
    ...
    $('.component,.component-content').css({
        'cursor': 'default',
        'outline': 'none'
    });
    ...
    $('.obj-custom-html iframe, .obj-custom-html div:eq(0)').css({
        'width': '100%',
        'height': '100%'
    });
        ...
        try {
            ...
            if (countdownClass != undefined) {
                $thisElm.find('.' + countdownClass).timeTo({
                    ...
                    if (expire_type == 'show-timer') {} else if (expire_type == 'hide-timer') {
    ...
    $videoAll_obj.find('[data-module="video"]').each(function(index, el) {
        if ($(this).find('iframe').length > 0) {} else if ($(this).find('video').length > 0) {
